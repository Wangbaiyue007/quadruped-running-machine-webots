...
{
  //------------------------------------------------------------------------参数区
  robot.spring_normal_length = 1.1;                                            //弹簧原长
  robot.d_length = 0.45;                                                       //躯长
  robot.d_width = 0.2;                                                         //躯宽
  robot.d_center = pow((pow(robot.d_length, 2) + pow(robot.d_width, 2)), 0.5); //腿和重心距离
  robot.v = 2;                                                                 //机器人水平运动速度
  robot.r_threshold = 0.95;                                                    //用于状态机切换的腿长阈值系数
  robot.k_spring = 2500;                                                       //弹簧刚度
  robot.F_thrust = 100.0;                                                      //THRUST推力，用于调节跳跃高度
  robot.F_shorten = -200.0;                                                    //收缩时的力
  robot.k_leg_p = 8;                                                           //腿部控制时的kp
  robot.k_leg_v = 0.8;                                                         //腿部控制时的kv
  robot.k_xz_dot = 0.01;                                                       //净加速度系数
  robot.k_pose_p = 4;                                                          //姿态控制时的kp
  robot.k_pose_v = 0.5;                                                        //姿态控制时的kv
  robot.k_equalize = 40;                                                       //平衡腿长时的k
  ...
  //腿偏移位置
  easyMat_create(&robot.legTransl, 3, 1);
  robot.legTransl.data[0][0] = 0.45;
  robot.legTransl.data[1][0] = 0;
  robot.legTransl.data[2][0] = 0.2;
  //关节空间
  robot.jointPoint1.r = 0.6;
  robot.jointPoint1.X_motor_angle = 0;
  robot.jointPoint1.Z_motor_angle = 0;

  robot.jointPoint2.r = 0.6;
  robot.jointPoint2.X_motor_angle = 0;
  robot.jointPoint2.Z_motor_angle = 0;

  robot.jointPoint3.r = 0.6;
  robot.jointPoint3.X_motor_angle = 0;
  robot.jointPoint3.Z_motor_angle = 0;

  robot.jointPoint4.r = 0.6;
  ...
  robot.is_foot_touching_a = true; //足底是否触地
  robot.is_foot_touching_b = true;
  ...
  robot.eulerAngle_dot_desire = 0; //yaw轴期望角速度
  robot.system_ms = 0;             //从仿真启动开始的计时器
  robot.stateMachine = FLIGHT_B;   //状态机
}
/*
机器人内存空间释放
*/
...
void updateRobotState()
{
  ...
  /*足底传感器更新*/
  robot.is_foot_touching_a = is_foot_touching1() && is_foot_touching4();

  robot.is_foot_touching_b = is_foot_touching2() && is_foot_touching3();
  ...
  easyMat_RPY(&robot.R_H_B, robot.eulerAngle.roll, robot.eulerAngle.pitch, 0);
...
void equalizeA()
{
  ...
  double dx2 = robot.spring_normal_length - robot.jointPoint4.r;
  double F_spring1 = dx1 * robot.k_spring - robot.k_equalize * (robot.jointPoint1.r - robot.jointPoint4.r) / 2;
  double F_spring2 = dx2 * robot.k_spring - robot.k_equalize * (robot.jointPoint4.r - robot.jointPoint1.r) / 2;
  set_spring_force1(F_spring1);
  set_spring_force4(F_spring2);
}
void equalizeA_thrust(double dF)
{
  ...
  set_spring_force4(F_spring2 + dF);
}
void equalizeB()
{
  double dx3 = robot.spring_normal_length - robot.jointPoint3.r; //求压缩量
  double dx2 = robot.spring_normal_length - robot.jointPoint2.r;
  double F_spring3 = dx3 * robot.k_spring - robot.k_equalize * (robot.jointPoint3.r - robot.jointPoint2.r) / 2;
  double F_spring2 = dx2 * robot.k_spring - robot.k_equalize * (robot.jointPoint2.r - robot.jointPoint3.r) / 2;
...
void equalizeB_thrust(double dF)
{
  ...
  double F_spring3 = dx3 * robot.k_spring - robot.k_equalize * (robot.jointPoint3.r - robot.jointPoint2.r) / 2;
  double F_spring2 = dx2 * robot.k_spring - robot.k_equalize * (robot.jointPoint2.r - robot.jointPoint3.r) / 2;
  set_spring_force3(F_spring3 + dF);
...
void shortenA()
{
  set_spring_force1(robot.F_shorten);
  set_spring_force4(robot.F_shorten);
}
void shortenB()
{
  set_spring_force3(robot.F_shorten);
  set_spring_force2(robot.F_shorten);
}

/*
Don't move the hip motor
*/
void stopA()
{
  set_X_velocity1(0.0);
...
void positionA()
{
  double Tx1, Tz1, Tx4, Tz4;
  double r = robot.jointPoint1.r;
  double w_yaw = robot.eulerAngle_dot.yaw * PI / 180; // Angular velocity
  w_yaw = w_yaw - robot.eulerAngle_dot_desire;
  double x_f1 = (robot.x_dot + w_yaw * robot.d_width) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.x_dot - robot.x_dot_desire);
  double z_f1 = (robot.z_dot - w_yaw * robot.d_length) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.z_dot - robot.z_dot_desire) - 0.0; //bend inside towards COM
  double y_f1 = -sqrt(r * r - x_f1 * x_f1 - z_f1 * z_f1);
  double x_f4 = (robot.x_dot - w_yaw * robot.d_width) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.x_dot - robot.x_dot_desire);
  double z_f4 = (robot.z_dot + w_yaw * robot.d_length) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.z_dot - robot.z_dot_desire) + 0.0;
  double y_f4 = -sqrt(r * r - x_f4 * x_f4 - z_f4 * z_f4);
  //printf("xf1: %f, zf1: %f, xf4: %f, zf4: %f\n", x_f1, z_f1, x_f4, z_f4); // Examine desired position
  ...
  //printf("x_angle1: %f, z_angle1: %f, x_angle4: %f, z_angle4: %f\n", x_angle_desire, z_angle_desire, x_angle_desire4, z_angle_desire4);
...
void positionB()
{
  double Tx2, Tz2, Tx3, Tz3;
  double r = robot.jointPoint3.r;
  double w_yaw = robot.eulerAngle_dot.yaw * PI / 180;
  w_yaw = w_yaw - robot.eulerAngle_dot_desire;
  double x_f2 = (robot.x_dot + w_yaw * robot.d_width) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.x_dot - robot.x_dot_desire);
  double z_f2 = (robot.z_dot + w_yaw * robot.d_length) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.z_dot - robot.z_dot_desire) - 0.0; //bend inside towards COM
  double y_f2 = -sqrt(r * r - x_f2 * x_f2 - z_f2 * z_f2);
  double x_f3 = (robot.x_dot - w_yaw * robot.d_width) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.x_dot - robot.x_dot_desire);
  double z_f3 = (robot.z_dot - w_yaw * robot.d_length) * robot.Ts / 2.0 + robot.k_xz_dot * (robot.z_dot - robot.z_dot_desire) + 0.0;
...
void update_xz_dot_desire()
{
  ...
  {
    robot.eulerAngle_dot_desire = -0.05;
    break;
  }
...
void update_xz_dot()
{
  ...
  if (robot.stateMachine == COMPRESSION_A || robot.stateMachine == THRUST_A || robot.stateMachine == UNLOADING_A || robot.stateMachine == LOADING_A)
  {
    ...
    //转换到{H}坐标系下
    pre_x2 = robot.workPoint_H4.data[0][0];
    pre_z2 = robot.workPoint_H4.data[2][0];
    easyMat_mult(&robot.workPoint_H4, &robot.R_H_B, &robot.workPoint_B4);
    now_x2 = robot.workPoint_H4.data[0][0];
    now_z2 = robot.workPoint_H4.data[2][0];
    //求导
  ...
  else
  {
    ...
    //转换到{H}坐标系下
    pre_x = robot.workPoint_H3.data[0][0];
    pre_z = robot.workPoint_H3.data[2][0];
    easyMat_mult(&robot.workPoint_H3, &robot.R_H_B, &robot.workPoint_B3);
    now_x = robot.workPoint_H3.data[0][0];
    now_z = robot.workPoint_H3.data[2][0];
    //求导
    now_x_dot = -(now_x - pre_x) / (0.001 * TIME_STEP);
    now_z_dot = -(now_z - pre_z) / (0.001 * TIME_STEP);
    //滤波
  ...
  if (robot.stateMachine == COMPRESSION_A || robot.stateMachine == COMPRESSION_B || robot.stateMachine == THRUST_A || robot.stateMachine == THRUST_B)
  {
    ...
    //printf("robotX_dot: %f, robotZ_dot: %f\n", robot.x_dot, robot.z_dot);
  }
}
...
void updateRobotStateMachine()
{
  ...
  case LOADING_A:
  {
    if (robot.jointPoint_dot1.r < 0 && robot.jointPoint_dot4.r < 0 && robot.is_foot_touching_a)
  ...
  case THRUST_A:
  {
    if (robot.jointPoint1.r > robot.spring_normal_length * robot.r_threshold && robot.jointPoint4.r > robot.spring_normal_length * robot.r_threshold && robot.is_foot_touching_a)
      robot.stateMachine = UNLOADING_A;
    break;
  }
  case UNLOADING_A:
  {
    if (is_foot_touching1() == false && is_foot_touching4() == false)
  ...
  case LOADING_B:
  {
    if (robot.jointPoint_dot3.r < 0 && robot.jointPoint_dot2.r < 0 && robot.is_foot_touching_b)
  ...
  case THRUST_B:
  {
    if (robot.jointPoint3.r > robot.spring_normal_length * robot.r_threshold && robot.jointPoint2.r > robot.spring_normal_length * robot.r_threshold && robot.is_foot_touching_b)
      robot.stateMachine = UNLOADING_B;
    break;
  }
  case UNLOADING_B:
  {
    if (is_foot_touching3() == false && is_foot_touching2() == false)
